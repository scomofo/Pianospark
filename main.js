  ...
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': ["default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; media-src 'self' file:"]
      }
    });
  });
...
ipcMain.handle('stems:checkCache', async (event, filePath) => {
  ...
  for (var i = 0; i < stemNames.length; i++) {
    ...
    if (fs.existsSync(stemFile)) { stemPaths[stemNames[i]] = stemFile; }
    else { allExist = false; }
  }
...
ipcMain.handle('stems:separate', async (event, filePath) => {
  ...
  if (!fs.existsSync(demucsBin)) {
    throw new Error(demucsBinName + ' not found at ' + demucsBin + '.\nPlace the demucs binary in the resources/ folder.');
  }
  if (!fs.existsSync(modelFile)) {
    throw new Error('Model file not found at ' + modelFile + '.\nDownload ggml-model-htdemucs-6s-f16.bin from HuggingFace and place it in resources/.');
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    demucsProcess = spawn(demucsBin, [modelFile, filePath, outputDir + path.sep, String(numThreads)], { windowsHide: true });
    var stderr = '';
    var startTime = Date.now();

    demucsProcess.stdout.on('data', (data) => {
      var line = data.toString().trim();
      if (line && mainWindow) mainWindow.webContents.send('stems:progress', { type: 'stdout', line: line });
    });
    demucsProcess.stderr.on('data', (data) => {
      stderr += data.toString();
      var line = data.toString().trim();
      if (line && mainWindow) mainWindow.webContents.send('stems:progress', { type: 'stderr', line: line });
    });
    demucsProcess.on('error', (err) => { demucsProcess = null; reject(new Error('Failed to start demucs: ' + err.message)); });
    demucsProcess.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        resolve({ stemPaths: stemPaths, elapsed: Math.round((Date.now() - startTime) / 1000) });
      } else {
...
ipcMain.on('stems:cancel', () => {
  if (demucsProcess) { demucsProcess.kill(); demucsProcess = null; }
});
