/* PianoSpark – recommend/candidates.js */
/* Collects recommendation candidates from multiple sources */
(function(){
  ...
  function getCurriculumCandidates(appType){
    ...
    var completedLessons = getCompletedLessonsList();
    var nextLessonId = typeof getNextLessonFromCurriculum === "function"
      ? getNextLessonFromCurriculum(curriculumId, completedLessons) : null;
    if(nextLessonId){
      var lesson = typeof getCurriculumItem === "function"
        ? getCurriculumItem("lessons", nextLessonId) : null;
      if(lesson){
        out.push({
          id: lesson.id,
          type: "lesson",
  ...
  function getWeakSpotCandidates(appType){
    ...
    // PianoSpark: add LH pattern weak spots
    for(var lh=0;lh<(weak.rhythm || []).length;lh++){
      out.push({
        id: "weak_lh_" + (weak.rhythm[lh].key || lh),
        type: "drill",
        title: "Practice LH pattern: " + (weak.rhythm[lh].key || "pattern"),
        source: "weakspot",
        targetSkill: "left_hand",
        level: 1,
        score: 0,
        reasons: ["Weak left-hand pattern"],
        meta: { weakSpot: weak.rhythm[lh].key }
      });
    }
    return out;
  }
  ...
  function getCompletedLessonsList(){
    return S.completedLessons || S.completedSessions || [];
  }
