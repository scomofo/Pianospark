/* PianoSpark – progression/mastery.js – Skill Mastery Tracking */
...
(function(){
  ...
  function updateMasteryFromPerformance(result){
    ...
    // PianoSpark-specific: scales and fingers
    if(result.scales){
      for(var s in result.scales){
        updateMastery("scales", s, result.scales[s]);
      }
    }

    if(result.fingers){
      for(var f in result.fingers){
        updateMastery("fingers", f, result.fingers[f]);
      }
    }
    ...
    if(result.lessonId){
      updateMastery("lessons", result.lessonId, result.accuracy);
    }
