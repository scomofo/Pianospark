(function(){
  ...
  function selectWarmupCandidate(){
    ...
    if(typeof FINGER_EXERCISES!=="undefined" && FINGER_EXERCISES.length){
      ...
      return {
        ...
        meta:{ exerciseId:ex.id, durationSec:120 }
      };
    }
    return {
      id:"warmup_default",
  ...
  window.selectWarmupItem = selectWarmupCandidate;
  window.selectWeakTransition = selectWeakTransitionCandidate;
  window.selectWeakPerformanceTarget = selectWeakPerformanceCandidate;
  window.selectFingerItem = selectFingerCandidate;

})();

// PianoSpark-specific: left-hand selector extension
(function(){

  function selectLeftHandCandidate(){
    var level = S.lhLevel || 1;
    return {
      id:"lh_level_" + level,
      type:"left_hand_pattern",
      priority:55 + Math.max(0, 6 - level),
      label:"Left-hand pattern practice",
      reason:"Support current LH progression",
      meta:{
        lhLevel:level
      }
    };
  ...
  buildPracticeCandidates = function(){
    ...
    var lh = selectLeftHandCandidate();
    if(lh) out.push(lh);
    out.sort(function(a,b){ return (b.priority||0) - (a.priority||0); });
    return out;
  };

  window.selectLeftHandCandidate = selectLeftHandCandidate;
  // Legacy alias
  window.selectLeftHandPatternItem = selectLeftHandCandidate;
