/* PianoSpark – onboarding/ui.js */
/* First-run onboarding screen renderer */
function onboardingFlowPage(){
  var step = getCurrentOnboardingStep();
  var h = '<div class="card mb16">';
  h += '<div><b>Setup</b></div>';
  h += '<div class="muted">Let\'s get PianoSpark ready for you.</div>';
  h += '</div>';
...
function renderOnboardingWelcome(){
  var h = '<div class="card mb16">';
  h += '<div><b>Welcome to PianoSpark</b></div>';
...
function renderOnboardingInstrument(){
  ...
  h += '<div style="margin-top:8px">Selected: '+escHTML(S.onboardingFlow.instrument || "none")+'</div>';
  h += '</div>';
  return h;
}
...
function renderOnboardingSkillLevel(){
  ...
  h += '<div style="margin-top:8px">Selected: '+escHTML(S.onboardingFlow.skillLevel || "none")+'</div>';
  h += '</div>';
  return h;
}

function renderOnboardingGoals(){
  var goals = S.onboardingFlow.goals || [];
  var opts = ["chords","rhythm","left_hand","melody","songs","technique"];
  var h = '<div class="card mb16">';
...
function renderOnboardingMidiSetup(){
  ...
  h += '<button onclick="act(\'toggle_midi\')">Toggle MIDI</button> ';
